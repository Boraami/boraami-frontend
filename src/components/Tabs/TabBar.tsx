...
import { SizableText, YStack, XStack, FontSizeTokens, Button, ScrollView } from "tamagui";
...
const SecondRoute: React.FC = () => (
  <ScrollView showsVerticalScrollIndicator={false}>
    <SizableText alignSelf="center" color={"$quoted-post-username"}>
      Tab 2
    </SizableText>
  </ScrollView>
);
...
const TabsView: React.FC = () => {
  ...
  const renderTabBar = (
    props: SceneRendererProps & { navigationState: { routes: TabRoute[] } }
  ) => {
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {props.navigationState.routes.map((route, i) => {
            const colorText = route.disabled
              ...
              ? "$tab-active-text"
              : "$tab-default-text";
            const colorIcon = route.disabled
              ? isDarkTheme
            ...
            return (
              <YStack key={i} marginHorizontal={5}>
                <Button
                  key={index}
